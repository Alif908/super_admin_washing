...
part 'add_package.dart';    // ← add package dialog
part 'register_hub.dart';   // ← register hub dialog
part 'add_coupon.dart';     // ← add coupon dialog
part 'add_device.dart';     // ← add device dialog
part 'download_report.dart';// ← csv download
...
// ↓ mix in _AddPackageMixin — _showAddPackageDialog now available in _State
class _State extends State<FleetDashboardScreen>
    with _AddPackageMixin, _RegisterHubMixin, _AddCouponMixin, _AddDeviceMixin, _DownloadReportMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final tsz = sw < 360 ? 18.0 : sw < 400 ? 20.0 : 24.0;
    final ssz = sw < 360 ? 11.0 : sw < 400 ? 12.0 : 13.0;
    final lsz = sw < 360 ? 9.0  : sw < 400 ? 10.0 : 11.0;
    final vsz = sw < 360 ? 24.0 : sw < 400 ? 28.0 : 32.0;
    final gap = sh * 0.015;
              ...
              _Card(
                child: Column(
                  ...
                  children: [
                    ...
                    _TRow(
                      cells: const ['Deep Clean', 'High pressure 50-min deep clean'],
                      flex: const [2, 3],
                      ssz: ssz,
                    ...
                    _Btn(
                      label: '+ Add Package',
                      onTap: () => _showAddPackageDialog(context, ssz), // ← works!
                    ),
                  ],
                ),
              ),
              ...
              _Card(
                child: Column(
                  ...
                  children: [
                    ...
                    _TRow(
                      cells: const ['HUB001', 'Calicut Express Wash Hub', 'Vimal'],
                      flex: const [2, 3, 2],
                      ssz: ssz,
                    ...
                    const _SBar(),
                    SizedBox(height: gap),
                    _Btn(label: '+ Register New Hub', onTap: () => _showRegisterHubDialog(context, ssz)),
                  ],
                ),
              ),
              SizedBox(height: gap),
              ...
              _Card(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: gap),
                    _THead(
                      cols: const ['COUPON CODE', 'DISCOUNT (%)', 'MAX / USER'],
                      flex: const [3, 3, 2],
                      lsz: lsz,
                    ),
                    _TRow(
                    ...
                    const _SBar(),
                    SizedBox(height: gap),
                    _Btn(label: '+ Add Coupon', onTap: () => _showAddCouponDialog(context, ssz)),
                  ],
                ),
              ),
              SizedBox(height: gap * 1.4),
                ...
                _Card(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: gap),
                      _THead(
                        cols: const ['DEVICE ID', 'DEVICE NAME', 'COND.'],
                        flex: const [2, 4, 2],
                        lsz: lsz,
                      ),
                      _TRow(
                        cells: const ['DEV001', 'Pressure Washer Pro 4000', 'Good'],
                        flex: const [2, 4, 2],
                        ssz: ssz,
                        bold: const [true, false, false],
                      ),
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 8),
                          _Btn(label: '+ Add Device', onTap: () => _showAddDeviceDialog(context, ssz)),
                        ],
                      ),
                    ],
                  ),
                ...
                _Card(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: gap),
                      _THead(
                        cols: const ['ORDER ID', 'DEVICE ID', 'DEVICE NAME'],
                        flex: const [2, 3, 4],
                        lsz: lsz,
                      ),
                      _TRow(
                        cells: const ['2', 'DEV001', 'Pressure Washer Pro 4000'],
                        flex: const [2, 3, 4],
                        ssz: ssz,
                        bold: const [false, true, true],
                      ),
                      _TRow(
                        cells: const ['1', 'DEV001', 'Pressure Washer Pro 4000'],
                        flex: const [2, 3, 4],
                        ssz: ssz,
                        bold: const [false, true, true],
                      ),
                ...
                _Card(
                  child: Column(
                    ...
                    children: [
                      ...
                      _FieldDrop(
                        value: _svcHub,
                        items: const ['-- Select Hub --', 'Calicut Express Wash Hub'],
                        fsz: ssz,
                        onChanged: (v) {
                      ...
                      _TRow(
                        cells: const ['1', 'Calicut Express Wash Hub', '9847022334'],
                        flex: const [2, 4, 3],
                        ssz: ssz,
...
class _TabBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fsz = sw < 360 ? 9.5 : sw < 400 ? 10.5 : 11.5;
    return Expanded(
      child: GestureDetector(
        onTap: onTap,
...
class _CTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(
    text,
    style: TextStyle(color: Colors.white, fontSize: ssz + 2, fontWeight: FontWeight.bold),
  );
}
...
class _InfoBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: _field, borderRadius: BorderRadius.circular(8)),
    child: Text(
      text,
      textAlign: TextAlign.center,
      ...
      children: List.generate(
        ...
        (i) => Expanded(
          ...
          child: Text(
            ...
            overflow: TextOverflow.ellipsis,
            maxLines: 2,
            style: TextStyle(color: _dim, fontSize: lsz, fontWeight: FontWeight.bold),
          ),
        ),
      ),
    ),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      ...
      Expanded(
        ...
        child: Container(
          height: 4,
          decoration: BoxDecoration(color: _field, borderRadius: BorderRadius.circular(4)),
        ),
      ),
    ],
  );
...
class _Btn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sw = MediaQuery.of(context).size.width;
    final fs = sw < 360 ? 10.0 : sw < 400 ? 11.5 : 13.0;
...
class _OutlineBtn extends StatelessWidget {
  ...
  final VoidCallback onTap;
  final double fsz;
  const _OutlineBtn({required this.label, required this.onTap, required this.fsz});
  @override
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Text(
        label,
        textAlign: TextAlign.center,
        style: TextStyle(color: Colors.white, fontSize: fsz, fontWeight: FontWeight.w500),
      ),
    ),
  );
}
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<String>(
        ...
        isExpanded: true,
        dropdownColor: _field,
        style: TextStyle(color: Colors.white, fontSize: fsz, fontFamily: 'Poppins'),
        icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white, size: 16),
        isDense: true,
        onChanged: onChanged,
